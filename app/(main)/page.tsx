...
export default function CivicSnap() {
  ...
  useEffect(() => {
    ...
    // Skip enableDebug as it's not available in current react-geocode version
    
    // Set a small delay before making API calls to ensure proper initialization
  ...
  // Use the react-geolocated hook for more reliable location detection
  const { coords, isGeolocationAvailable, isGeolocationEnabled, getPosition, positionError } = useGeolocated({
    ...
    watchPosition: false,
    suppressLocationOnMount: true, // Don't get location on mount, let user trigger it
    onError: (error) => {
      ...
      // Enhanced handling for empty error object case which causes the "Geolocation error: {}" error
      if (!error || (typeof error === 'object' && Object.keys(error).length === 0)) {
        console.warn("Empty geolocation error object received");
        setLocationError("Location services unavailable. Please check your browser permissions and try again, or enter location manually.");
        setUseManualLocation(true); // Automatically switch to manual location entry
      } else {
...
clearTimeout(window._locationTimeoutId as number);
  ...
  const getCurrentLocation = () => {
    ...
    if (!isGeolocationEnabled) {
      setLocationError("Location services are disabled. Please enable them in your browser settings.");
      setUseManualLocation(true);
      setLocationLoading(false);
      return;
    }
    ...
    try {
      ...
      // Set a timeout to handle cases where getPosition might silently fail
      window._locationTimeoutId = window.setTimeout(() => {
        // If this executes, it means getPosition didn't call onSuccess or onError
      ...
      // Call getPosition - the timeout will be cleared in onSuccess or onError callbacks
      getPosition();
    } catch (err) {
      console.error("Unexpected error in geolocation:", err);
      setLocationError("Unexpected error getting location. Please try again or enter manually.");
      setUseManualLocation(true);
      setLocationLoading(false);
...
if (mediaFile) {
  ...
  const handleSubmit = async () => {
    ...
    // Validate that we have at least issue text or location
    if (!issueText && !manualLocation.trim() && (!location || typeof location.lat !== 'number' || typeof location.lng !== 'number')) {
      alert("Please provide either a description of the issue or your location");
      return;
    }

    // Validate location information if user chose manual location
    if (useManualLocation) {
      if (!manualLocation.trim() && !issueText) {
        setLocationError("Please enter your location in the text field or provide issue description");
        return;
      }
    } else {
      if ((!location || typeof location.lat !== 'number' || typeof location.lng !== 'number') && !issueText) {
        setLocationError("Location information is missing or invalid. Please try getting your location again, enter it manually, or provide issue description.");
        return;
      }
    }
        ...
        <AnimatePresence>
          {step === "capture" && (
            ...
            >
              ...
              <h1 className="text-3xl font-bold text-white mb-2">CivicReport</h1>
            ...
            >
              ...
              <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Button
                  ...
                  disabled={(!issueText && !location && !manualLocation.trim())}
            ...
            >
              ...
              <div className="space-y-2 text-slate-300">
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
                  🤖 Processing your information with Gemini AI...
                </motion.p>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}>
                  📍 Identifying relevant authorities based on your description and location...
                </motion.p>
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.5 }}>
                  🐦 Generating optimized tweet for your issue...
                </motion.p>
              </div>
            </motion.div>
          )}
